...
$records = mysqli_num_rows(mysqli_query($conn," select * from goods where category = 'B' "));
...
$sql = sprintf("select * from goods where category = 'B' limit %d,4",($cpage-1)*4);
...
<body>
<center>
<font size="5" color="purple">服装</font></br>
...
<table border="1" width=450 bgcolor="#fff0f5">
    <tr>
        <td>总共<?=$records?>条商品信息 共<?=$ptotal?>页 当前第<?=$cpage?>页
            <a href="show_fuzhuang.php?p=1">首页</a>
            <a href="show_fuzhuang.php?p=<?=($cpage-1)?>">上一页</a>
            <a href="show_fuzhuang.php?p=<?=($cpage+1)?>">下一页</a>
            <a href="show_fuzhuang.php?p=<?=$ptotal?>">尾页</a>
        </td>
    </tr>
</table>
